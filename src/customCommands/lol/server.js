...
module.exports = {
  name: 'lol search',
  description: 'Busca por jogadores de lol que estejam no servidor.',
  category: '🎮 Jogos',
  usage: '<elo> [role] [main]',
  async run (client, message, args) {
    ...
    try {
      let data = {guild: message.member.guild, userId: message.author.id, elo: elo.toLowerCase(), role: role.toLowerCase(), main: main.toLowerCase()};
      let account = await GuildLolController.searchAccountOnServer(data);
      
      if(!account) return message.channel.send(`<@${message.author.id}>, nenhum usuário neste servidor foi encontrado!`);
